...
const script: PlutusScript = {
    code: cbor
        .encode(
            Buffer.from(
                plutusScript.validators.filter((val: any) => val.title === "vesting.vesting.spend")[0].compiledCode,
                "hex"
            )
        )
        .toString("hex"),
...
export default function Home() {
    ...
    const [adaQuantity, setAdaQuantity] = useState(50);
	...
	navigator.clipboard.writeText(script.code).then(() => {
		...
		<div className="min-h-screen bg-[#1f1f1f] text-white flex flex-col">
			...
			<main className="flex-grow container mx-auto px-4 py-8">
				<h1 className="text-4xl font-extrabold text-center text-[#c80e22] mb-6">Vesting</h1>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
					...
					<div className="card border border-white rounded-lg p-6 shadow-md hover:shadow-lg transition duration-300 bg-[#1f1f1f]">
						...
						<UnlockButton setState={setState} state={state} txLockHash={txLockHash} lockUntilTimestamp={lockUntilTimestamp} />
					</div>
				</div>
...
function UnlockButton({ setState, state, txLockHash, lockUntilTimestamp }: ButtonParamsUnlock) {
...
function getPlutusScript(): PlutusScript {
    return {
        code: cbor
            .encode(Buffer.from(plutusScript.validators.filter((val: any) => val.title == "vesting.vesting.spend")[0].compiledCode, "hex"))
            .toString("hex"),
        version: "V3",
    };
}
